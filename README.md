...
At startup, the user keeps their face straight while the system records a neutral head pose. Future pose angles are measured relative to this calibrated centre. A non-stop direction must remain consistent for several frames before it is shown as a command. If face tracking is lost, the output immediately becomes `STOP`.
...
```python
HEAD_YAW_THRESHOLD = 15.0      # sensitivity for LEFT / RIGHT
HEAD_PITCH_THRESHOLD = 12.0    # sensitivity for FORWARD / BACKWARD
COMMAND_CONFIRM_FRAMES = 8     # repeated frames required before movement command
